function freeze_pane_listener(what_is_this, table_class) {
  ...
  return function () {
    ...
    var translate_xy =
      "translate(" +
      what_is_this.scrollLeft +
      "px," +
      what_is_this.scrollTop +
      "px)";

    var fixed_vertical_elts = document.getElementsByClassName(
      table_class + " freeze_vertical"
    );
    var fixed_horizontal_elts = document.getElementsByClassName(
      table_class + " freeze_horizontal"
    );
    var fixed_both_elts = document.getElementsByClassName(
      table_class + " freeze"
    );
...
function parent_class(wanted_node_name, elt) {
  // Function to work up the DOM until it reaches
  // an element of type wanted_node_name, and return
  // that element's id.
...
var i, parent_div_id, parent_tr, table_i, scroll_div, parent_div_class;
var scrolling_table_div_ids = [
  "scrolling_table_1",
  "scrolling_table_2",
  "freeze_1",
  "freeze_2",
...
var fixed_horizontal_elements =
  document.getElementsByClassName("freeze_horizontal");
...
for (i = 0; i < scrolling_table_div_ids.length; i++) {
  ...
  if (scroll_div) {
    scroll_div.addEventListener(
      "scroll",
      freeze_pane_listener(scroll_div, scrolling_table_div_ids[i])
    );
  }
}

...
class M_air extends CI_Model {
    ...
    function cek_header($mdl1_air,$mdl1_urutan, $mdl1_tgl_efektif){
        $this->db1->from($this->tabel1);
        $this->db1->where('air', $mdl1_air);
        $this->db1->where('urutan', $mdl1_urutan);
        $this->db1->where('tgl_efektif', $mdl1_tgl_efektif);
        return $this->db1->get();
    }

    function cek_header2($mdl1_air, $mdl1_urutan, $mdl1_tgl_efektif, $mdl1_headerid){
        $this->db1->from($this->tabel1);
        $this->db1->where('air', $mdl1_air);
        $this->db1->where('urutan', $mdl1_urutan);
        $this->db1->where('tgl_efektif', $mdl1_tgl_efektif);

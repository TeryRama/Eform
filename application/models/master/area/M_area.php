...
class M_area extends CI_Model {
    ...
    function cek_header($mdl1_area, $mdl1_tgl_efektif){
        $this->db1->from($this->tabel1);
        $this->db1->where('area', $mdl1_area);
        $this->db1->where('tgl_efektif', $mdl1_tgl_efektif);
        return $this->db1->get();
    }

    function cek_header2($mdl1_area, $mdl1_tgl_efektif, $mdl1_headerid){
        $this->db1->from($this->tabel1);
        $this->db1->where('area', $mdl1_area);
        $this->db1->where('tgl_efektif', $mdl1_tgl_efektif);

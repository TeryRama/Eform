...
class M_dept_pengguna extends CI_Model {
		
	var $tabel1 = 'tblmst_dept_pengguna';
    ...
    function cek_header($mdl1_dept_pengguna, $mdl1_tgl_efektif){
        $this->db1->from($this->tabel1);
        $this->db1->where('dept_pengguna', $mdl1_dept_pengguna);
        $this->db1->where('tgl_efektif', $mdl1_tgl_efektif);
        return $this->db1->get();
    }

    function cek_header2($mdl1_dept_pengguna, $mdl1_tgl_efektif, $mdl1_headerid){
        $this->db1->from($this->tabel1);
        $this->db1->where('dept_pengguna', $mdl1_dept_pengguna);
        $this->db1->where('tgl_efektif', $mdl1_tgl_efektif);

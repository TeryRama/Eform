<?php
class M_formfrmfss054_04 extends CI_Model
{
    ...
    function get_tanggal()
    {
        return $this->db1->query("SELECT
                                    ...
                                    EXTRACT ( MONTH FROM create_date ) bulan
                                FROM
                                    tblfrmfrmfss031hdr")->result();
        // WHERE
        //     EXTRACT ( MONTH FROM create_date ) = '$bulan' 
        //     AND nama_mesin = '$nama_mesin' 
        //     AND kode_mesin = '$kode_mesin'")->result();
    }
    ...
    function get_list_mesin_by($deptabbr, $item)
    {
        return $this->db1->query("SELECT
                                    b.item2 gugus,
                                    d.detail_id,
                                    d.nama_mesin,
                                    d.kode_mesin,
                                    TO_CHAR( f.create_date, 'DD-MM-YYYY' ) tanggal_perawatan,
                                    EXTRACT ( MONTH FROM f.create_date ) bulan
                                FROM
                                    tblmstformitemmesin_hdr A 
                                LEFT JOIN
                                    tblmstformitemmesin_dtl_b b 
                                ...
                                LEFT JOIN
                                    tblfrmfrmfss031hdr f
                                    on f.nama_mesin = d.nama_mesin
                                    and f.kode_mesin = d.kode_mesin
                                WHERE
                                    A.departemen = '$deptabbr' 
                                    AND e.item = '$item' 
                                ORDER BY
                                    d.detail_id")->result();
    }
    function get_list_panel_by($deptabbr, $item)
    {
        return $this->db1->query("SELECT
    ...
    function check2($create_date, $docno, $rev, $dept, $deptabbr, $gugus, $tahun, $headerid, $item)
    {
        $this->db1->from($this->tabel1);
        $this->db1->where('create_date', $create_date);
    ...
    function fn_update_frmwtd054($update_dept, $update_item, $update_tahun)
    {
        $this->db1->trans_begin();
        $query = $this->db1->query("SELECT * FROM fn_update_frmwtd054('$update_dept','$update_item','$update_tahun')");
        if ($this->db1->trans_status() == FALSE) {
            $this->db1->trans_rollback();
            $result = 0;
        } else {

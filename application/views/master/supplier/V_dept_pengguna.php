        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">DATA DEPARTEMEN (PENGGUNA)</h4>
                </div>
                                    ...
                                    <?php if (!empty($list_data)) {
                                        ...
                                        foreach ($list_data as $list_data_row) {
                                            ...
                                            <tr>
                                                ...
                                                <td align="center"><?= $list_data_row->dept_pengguna ?></td>
                                                ...
                                                <td align="center">
                                                    <a href="<?= base_url('master/area/C_area/delete/' . $list_data_row->headerid) ?>" target="_self" title="Delete Data" onClick="return confirm('ANDA YAKIN AKAN MENGHAPUS DATA PENTING INI ... ?')"><span class="btn bg-gradient-danger feather icon-trash-2"></span></a>
                                                </td>
                                            </tr>
                                    <?php }
                                    } ?>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-8">
                                        <input type="text" name="mdl1_dept_pengguna" id="mdl1_dept_pengguna" class="form-control mdl1_dept_pengguna set_kosong" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-md-4">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.modal_forminput', function() {
            ...
            if ($(this).attr("name") == 'btn_update' && headerid != '') {
                $("#modal_form").attr("action", "<?= base_url('master/supplier/C_dept_pengguna/form/update') ?>");
                $("#mdl1_headerid").val(headerid);

                $.ajax({
                    url: "<?= base_url(); ?>/master/supplier/C_dept_pengguna/get_dt_update",
                    type: "post",
                    data: {
                        headerid
                    },
                    success: function(data) {
                        if (JSON.parse(data)['status'] == 0) {
                            $(".mdl1_dept_pengguna").val(JSON.parse(data)['data'][0].dept_pengguna);
                ...
                $("#modal_form").attr("action", "<?= base_url('master/supplier/C_dept_pengguna/form/add') ?>");
                $("#mdl1_headerid").val('');

        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">DATA Trafo</h4>
                </div>
                                ...
                                <?php if(!empty($list_data)){ 
                                  ...
                                  foreach($list_data as $list_data_row){
                                    ...
                                    <tr>
                                      ...
                                      <td align="center"><?= $list_data_row->trafo ?></td>
                                      <td align="center"><?= $list_data_row->nama_trafo ?></td>
                                      <td align="center"><?= $list_data_row->read_ct_trafo ?></td>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group row">
                                    <div class="col-md-4">
                                        <span>Kode Trafo</span>
                                    </div>
                                    <div class="col-md-8">
                                        <input type="text" name="mdl1_trafo" id="mdl1_trafo" class="form-control mdl1_trafo set_kosong" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-8">
                                        <input type="text" name="mdl1_nama_trafo" id="mdl1_nama_trafo" class="form-control mdl1_nama_trafo set_kosong" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-md-4">
                                        <span>Reading CT Trafo</span>
                                    </div>
                                    <div class="col-md-8">
                                        <input type="text" name="mdl1_read_ct_trafo" id="mdl1_read_ct_trafo" class="form-control mdl1_read_ct_trafo set_kosong" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-md-4">
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.modal_forminput', function(){
            ...
            if($(this).attr("name")=='btn_update' && headerid!=''){
                $("#modal_form").attr("action", "<?= base_url('master/trafo/C_trafo/form/update') ?>");
                $("#mdl1_headerid").val(headerid);

                $.ajax({
                    url : "<?= base_url();?>/master/trafo/C_trafo/get_dt_update",
                    type :"post",
                    data : { headerid },
                    success : function(data){
                        if(JSON.parse(data)['status']==0){
                            $(".mdl1_trafo").val(JSON.parse(data)['data'][0].trafo);
                            $(".mdl1_nama_trafo").val(JSON.parse(data)['data'][0].nama_trafo);
                            $(".mdl1_read_ct_trafo").val(JSON.parse(data)['data'][0].read_ct_trafo);
                ...
                if($(this).attr("id")=='btn_update' && confirm('Ubah data?')){
                    $("#judul_modal").empty().append('Edit Trafo');
                    $("#ini_modal").modal();
                }else{
                    $("#judul_modal").empty().append('Lihat Trafo');
                    $('.btn_disable').prop('disabled', true);
                    $("#ini_modal").modal();                    
                }
            }else if($(this).attr("name")=='btn_add'){
                $("#modal_form").attr("action", "<?= base_url('master/trafo/C_trafo/form/add') ?>");
                $("#mdl1_headerid").val('');
                $("#judul_modal").empty().append('Tambah Trafo');

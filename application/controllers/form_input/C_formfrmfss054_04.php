...
class C_formfrmfss054_04 extends CI_Controller
{
    ...
    function get_docno()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $docno        = 'RP/' . date("y", strtotime($create_date)) . '/' . $arr_bulan[(float)date("m", strtotime($create_date))] . '/' . date("d", strtotime($create_date));
    ...
    function get_list_mesin_by()
    {
        if ($this->session->userdata('logged_in')) {
            $session_data   = $this->session->userdata('logged_in');
            $dept           = $this->input->post('dept');
            $item           = $this->input->post('item');
            $dthasil        = $this->model->get_list_mesin_by($dept, $item);
    ...
    function get_list_panel_by()
    {
        if ($this->session->userdata('logged_in')) {
            $session_data   = $this->session->userdata('logged_in');
            $dept           = $this->input->post('dept');
            $item           = $this->input->post('item');
            $dthasil        = $this->model->get_list_panel_by($dept, $item);
    ...
    function form()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $dtl_a_gugus        = $this->input->post('dtl_a_gugus');
            ...
            if ($frmaksi == 'dtsave') {
                $cekheader = $this->model->check($create_date, $docno, $rev, $dept, $deptabbr, $gugus, $tahun, $item);

                // cek kalau create date dan docno sudah pernah disimpan
                if ($cekheader->num_rows() > 0) {
                    // pesan gagal krn data sudah ada
                    $data['message']          = 'Gagal, Data pada tanggal Laporan : ' . $create_date . ', No Dokumen : ' . $docno . ', Revisi : ' . $rev . ', Dept : ' . $deptabbr . ', Item :' . $item . ', Gugus : ' . $gugus . ' dan Tahun : ' . $tahun . ' sudah pernah disimpan';
                    ...
                    for ($i = 0; $i < $jml; $i++) {
                        ...
                        $data6 = array(
                            ...
                            'gugus'      => $dtl_a_gugus[$i],
                            'month1'     => $dtl_a_month1[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month1[$i])),
                            'month2'     => $dtl_a_month2[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month2[$i])),
                            'month3'     => $dtl_a_month3[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month3[$i])),
                            'month4'     => $dtl_a_month4[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month4[$i])),
                            'month5'     => $dtl_a_month5[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month5[$i])),
                            'month6'     => $dtl_a_month6[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month6[$i])),
                            'month7'     => $dtl_a_month7[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month7[$i])),
                            'month8'     => $dtl_a_month8[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month8[$i])),
                            'month9'     => $dtl_a_month9[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month9[$i])),
                            'month10'    => $dtl_a_month10[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month10[$i])),
                            'month11'    => $dtl_a_month11[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month11[$i])),
                            'month12'    => $dtl_a_month12[$i] == '' ? NULL : date("Y-m-d", strtotime($dtl_a_month12[$i])),
                            'ket'        => $dtl_a_ket[$i],
                        );
                ...
                $data['list_gugus']   = $this->model->get_gugus_by($dept);
                ...
                if (isset($_POST['btnproses'])) {
                    ...
                    foreach ($data['dtheader'] as $headerrow) {
                        $update_tahun = $headerrow->tahun;
                        $update_dept = $headerrow->deptabbr;
                        $update_item = $headerrow->item;
                    }

                    $this->model->fn_update_frmwtd054($update_dept, $update_item, $update_tahun);

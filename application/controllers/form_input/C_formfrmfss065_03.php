...
class C_formfrmfss065_03 extends CI_Controller
{
    ...
    function get_frmfss060()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $dthasil        = $this->model->get_frmfss060($create_date, $dept, $nama_mesin);
    ...
    function form()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            if ($frmaksi == 'dtsave') {
                $cekheader = $this->model->check($create_date, $docno, $dept, $deptabbr, $gugus, $nama_mesin);
                    ...
                    for ($i = 0; $i < $jml; $i++) {
                        ...
                        if($dtl_a_tanggal[$i] != ''){
                            if(date("Y-m-d", strtotime($dtl_a_tanggal[$i])) != '1970-01-01'){
                                $vdtl_a_tanggal[$i]   = $dtl_a_tanggal[$i];
                            }else{
                ...
                if (isset($_POST['btnproses'])) {
                    $cekheader = $this->model->check2($create_date, $docno, $dept, $deptabbr, $gugus, $nama_mesin, $headerid);

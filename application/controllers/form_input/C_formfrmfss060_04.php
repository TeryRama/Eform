...
class C_formfrmfss060_04 extends CI_Controller
{
    ...
    function get_docno()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $docno        = 'RKDP/' . date("y", strtotime($create_date)) . '/' . $arr_bulan[(float)date("m", strtotime($create_date))] . '/' . date("d", strtotime($create_date));
    ...
    function get_frmfss812()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $result_frmfss812   = $this->model->get_frmfss812($bulan, $tahun, $dept);

            if (count($result_frmfss812) > 0) {
                $pesan = "Berhasil mengambil data!";
                $pesan .= !empty($result_frmfss812) ? "\n FRM-FSS-812 ✔" : "\n FRM-FSS-812 ✘";
                $hasil = array(
                    'status'  => 0,
                    'vstatus' => 'success',
                    'pesan'   => $pesan,
                    'result_frmfss812' => $result_frmfss812,
                );
            } else {
                $hasil = array(
                    'status'     => 1,
    ...
    function form()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $periode            = date("Y-m", strtotime(addslashes('01-' . $this->input->post('periode'))));
            ...
            $detail_id_812            = $this->input->post('detail_id_812');
            $headerid_812             = $this->input->post('headerid_812');
            $dtl_a_nama_mesin         = $this->input->post('dtl_a_nama_mesin');
            $dtl_a_kode_mesin         = $this->input->post('dtl_a_kode_mesin');
            $dtl_a_tgl_start          = $this->input->post('dtl_a_tgl_start');
            $dtl_a_waktu_start        = $this->input->post('dtl_a_waktu_start');
            $dtl_a_tgl_stop           = $this->input->post('dtl_a_tgl_stop');
            $dtl_a_waktu_stop         = $this->input->post('dtl_a_waktu_stop');
            $dtl_a_durasi             = $this->input->post('dtl_a_durasi');
            $dtl_a_status_perbaikan   = $this->input->post('dtl_a_status_perbaikan');
            $dtl_a_kategori           = $this->input->post('dtl_a_kategori');
            $dtl_a_planned            = $this->input->post('dtl_a_planned');
            $dtl_a_perbaikan          = $this->input->post('dtl_a_perbaikan');
            $dtl_a_analisa            = $this->input->post('dtl_a_analisa');
            $dtl_a_tindakan           = $this->input->post('dtl_a_tindakan');
            $dtl_a_keterangan         = $this->input->post('dtl_a_keterangan');


            if ($frmaksi == 'dtsave') {
                $cekheader = $this->model->check($create_date, $docno, $periode, $dept, $deptabbr, $kategori);
                ...
                if ($cekheader->num_rows() > 0) {
                    ...
                    $data['dtkategori']       = addslashes($this->input->post('kategori'));
                    ...
                    for ($i = 0; $i < $jml; $i++) {
                        ...
                        if ($dtl_a_tgl_start[$i] != '') {
                            if (date('Y-m-d', strtotime($dtl_a_tgl_start[$i])) != '1970-01-01') {
                                $vdtl_a_tgl_start[$i] = date('Y-m-d', strtotime($dtl_a_tgl_start[$i]));
                            } else {
                                $vdtl_a_tgl_start[$i] = NULL;
                            }
                        } else {
                            $vdtl_a_tgl_start[$i] = NULL;
                        }
                        if ($dtl_a_tgl_stop[$i] != '') {
                            if (date('Y-m-d', strtotime($dtl_a_tgl_stop[$i])) != '1970-01-01') {
                                $vdtl_a_tgl_stop[$i] = date('Y-m-d', strtotime($dtl_a_tgl_stop[$i]));
                            } else {
                                $vdtl_a_tgl_stop[$i] = NULL;
                            }
                        } else {
                            $vdtl_a_tgl_stop[$i] = NULL;
                        }
                        $data6 = array(
                            ...
                            'detail_id_812'    => $detail_id_812[$i],
                            'headerid_812'     => $headerid_812[$i],
                            'nama_mesin'       => $dtl_a_nama_mesin[$i],
                            'kode_mesin'       => $dtl_a_kode_mesin[$i],
                            'tgl_start'        => $vdtl_a_tgl_start[$i],
                            'waktu_start'      => $dtl_a_waktu_start[$i],
                            'tgl_stop'         => $vdtl_a_tgl_stop[$i],
                            'waktu_stop'       => $dtl_a_waktu_stop[$i],
                            'durasi'           => $dtl_a_durasi[$i],
                            'status_perbaikan' => $dtl_a_status_perbaikan[$i],
                            'kategori'        => $dtl_a_kategori[$i],
                            'planned'          => $dtl_a_planned[$i],
                            'perbaikan'        => $dtl_a_perbaikan[$i],
                            'analisa'          => $dtl_a_analisa[$i],
                            'tindakan'         => $dtl_a_tindakan[$i],
                            'keterangan'       => $dtl_a_keterangan[$i],
                ...
                if (isset($_POST['btnproses'])) {
                    $cekheader = $this->model->check2($create_date, $docno, $periode, $dept, $deptabbr, $kategori, $headerid);
                    ...
                    // $headerid    = $this->uri->segment(7);

                    $dtheader    = $this->model->get_header_byid($headerid); // manggil data header
                    $data7 = array('dtheader' => $dtheader);

                    foreach ($dtheader as $dtheaderrow) {
                        $update_bulan = explode('-', $dtheaderrow->periode)[1];
                        $update_tahun = explode('-', $dtheaderrow->periode)[0];
                        $update_dept = $dtheaderrow->dept;
                    }

                    $this->model->fn_update_frmutl060($update_bulan, $update_tahun, $update_dept);

...
class C_formfrmfss064_05 extends CI_Controller
{
    ...
    function get_docno()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $docno        = 'DIM/' . date("y", strtotime($create_date)) . '/' . $arr_bulan[(float)date("m", strtotime($create_date))] . '/' . date("d", strtotime($create_date));
    ...
    function form()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $rangkain           = $this->input->post('rangkain');
            $ket                = $this->input->post('ket');


            if ($frmaksi == 'dtsave') {
                $cekheader = $this->model->check($create_date, $docno, $rev, $dept, $deptabbr,$item);
                ...
                if (isset($_POST['btnproses'])) {
                    $cekheader = $this->model->check2($create_date, $docno, $rev, $dept, $deptabbr, $headerid, $item);

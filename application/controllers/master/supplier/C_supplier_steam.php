...
class C_supplier_steam extends CI_Controller {
    ...
    function __construct(){
        ...
        parent :: __construct();
        $this->load->model(array('M_user', 'M_menu','master/supplier/M_supplier_steam'));
        ...
        if ($this->session->userdata('logged_in')){
            ...
            $url_str      = uri_string();
            $akses_check  = $this->M_user->check_akses_bylevelid($leveluid,'C_supplier_steam');
    ...
    function index(){
        ...
        $data['Titel']          = 'Master - supplier_steam';
        ...
        $data['list_data']      = $this->M_supplier_steam->get_records();
        $data['list_kode_form'] = $this->M_supplier_steam->get_list_form();
        
        $this->load->view('master/supplier/V_supplier_steam', array_merge($data));
    }
    ...
    //         $dtpekerja_allinfo  = $this->M_supplier_steam->get_pekerja_allinfo($personalstatus, $nik);
    ...
    function form(){
        if($this->session->userdata('logged_in')){
            ...
            $mdl1_dept_steam            = $this->input->post('mdl1_dept_steam');   
            ...
            if ((isset($_POST['btnproses']) && $this->uri->segment(5)=='add') || (isset($_POST['btncopy']) && $this->uri->segment(5)=='update')){
                if($this->M_supplier_steam->cek_header($mdl1_dept_steam, $mdl1_tgl_efektif)->num_rows()==0){
                    $data['dept_steam']            = $mdl1_dept_steam; 
                    ...
                    if ($this->M_supplier_steam->insert_hdr($data)){
                    ...
                    echo '<script>alert("Gagal menyimpan!!!\nData dengan supplier_steam : '.$mdl1_dept_steam.', untuk tanggal : '.$mdl1_tgl_efektif.' sudah pernah disimpan.");</script>';
                }
               
            }else if (isset($_POST['btnproses']) && $this->uri->segment(5)=='update'){
                if($this->M_supplier_steam->cek_header2($mdl1_dept_steam, $mdl1_tgl_efektif, $mdl1_headerid)->num_rows()==0){
                    $data['dept_steam']            = $mdl1_dept_steam; 
                    $data['tgl_efektif']     = $mdl1_tgl_efektif;   
                    $data['form_penggunaan'] = $form_penggunaan;           
                    ...
                    if ($this->M_supplier_steam->update_hdr($mdl1_headerid, $data)){
                        echo "<script>alert('Data berhasil disimpan....!!!! ');</script>";
                    }
                }else{
                    echo '<script>alert("Gagal menyimpan!!!\nData dengan supplier_steam : '.$mdl1_dept_steam.', untuk tanggal : '.$mdl1_tgl_efektif.' sudah pernah disimpan.");</script>';
            ...
            redirect('master/supplier/C_supplier_steam', 'refresh');
        }else{
            //Jika tidak ada session di kembalikan ke halaman login
            redirect('C_login', 'refresh');
        }
    ...
    function get_dt_update(){   
        if($this->session->userdata('logged_in')) {
            ...
            $dtdetail     = $this->M_supplier_steam->get_records_by($headerid);
    ...
    function delete($id){
        if($this->session->userdata('logged_in')) {
            ...
            if ($this->M_supplier_steam->update_hdr($id, $data)){
                redirect('master/supplier/C_supplier_steam');
            }

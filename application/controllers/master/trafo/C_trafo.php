...
class C_trafo extends CI_Controller {
    ...
    function __construct(){
        ...
        parent :: __construct();
        $this->load->model(array('M_user', 'M_menu','master/trafo/M_trafo'));
        ...
        if ($this->session->userdata('logged_in')){
            ...
            $url_str      = uri_string();
            $akses_check  = $this->M_user->check_akses_bylevelid($leveluid,'C_trafo');
    ...
    function index(){
        ...
        $data['list_data']      = $this->M_trafo->get_records();
        $data['list_kode_form'] = $this->M_trafo->get_list_form();
        
        $this->load->view('master/trafo/V_trafo', array_merge($data));
    }
    ...
    //         $dtpekerja_allinfo  = $this->M_trafo->get_pekerja_allinfo($personalstatus, $nik);
    ...
    function form(){
        if($this->session->userdata('logged_in')){
            ...
            $mdl1_trafo           = $this->input->post('mdl1_trafo');
            $mdl1_read_ct_trafo   = $this->input->post('mdl1_read_ct_trafo');
            $mdl1_nama_trafo      = $this->input->post('mdl1_nama_trafo');
            ...
            if ((isset($_POST['btnproses']) && $this->uri->segment(5)=='add') || (isset($_POST['btncopy']) && $this->uri->segment(5)=='update')){
                if($this->M_trafo->cek_header($mdl1_trafo,$mdl1_nama_trafo,$mdl1_read_ct_trafo, $mdl1_tgl_efektif)->num_rows()==0){
                    $data['trafo']            = $mdl1_trafo;
                    $data['nama_trafo']       = $mdl1_nama_trafo;
                    $data['read_ct_trafo']    = $mdl1_read_ct_trafo;
                    ...
                    if ($this->M_trafo->insert_hdr($data)){
                    ...
                    echo '<script>alert("Gagal menyimpan!!!\nData dengan trafo : '.$mdl1_trafo. ', Nama trafo : ' . $mdl1_nama_trafo . ',Reading CT trafo : ' . $mdl1_read_ct_trafo . ', untuk tanggal : '.$mdl1_tgl_efektif.' sudah pernah disimpan.");</script>';
                }
               
            }else if (isset($_POST['btnproses']) && $this->uri->segment(5)=='update'){
                if($this->M_trafo->cek_header2($mdl1_trafo,$mdl1_nama_trafo,$mdl1_read_ct_trafo, $mdl1_tgl_efektif, $mdl1_headerid)->num_rows()==0){
                    $data['trafo']            = $mdl1_trafo;
                    $data['nama_trafo']       = $mdl1_nama_trafo;
                    $data['read_ct_trafo']    = $mdl1_read_ct_trafo;
                    $data['tgl_efektif']      = $mdl1_tgl_efektif;
                    ...
                    if ($this->M_trafo->update_hdr($mdl1_headerid, $data)){
                        echo "<script>alert('Data berhasil disimpan....!!!! ');</script>";
                    }
                }else{
                    echo '<script>alert("Gagal menyimpan!!!\nData dengan trafo : '.$mdl1_trafo.',Nama trafo : '.$mdl1_nama_trafo.',Reading CT trafo : '.$mdl1_read_ct_trafo.', untuk tanggal : '.$mdl1_tgl_efektif.' sudah pernah disimpan.");</script>';
            ...
            redirect('master/trafo/C_trafo', 'refresh');
        }else{
            //Jika tidak ada session di kembalikan ke halaman login
            redirect('C_login', 'refresh');
        }
    ...
    function get_dt_update(){   
        if($this->session->userdata('logged_in')) {
            ...
            $dtdetail     = $this->M_trafo->get_records_by($headerid);
    ...
    function delete($id){
        if($this->session->userdata('logged_in')) {
            ...
            if ($this->M_trafo->update_hdr($id, $data)){
                redirect('master/trafo/C_trafo');
            }

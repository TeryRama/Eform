...
class C_export_toexcel_frmfss031_02 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
    ...
    function exportxls()
    {
        ...
        $dtheader = $this->M_formfrmfss031_02->get_header_byid($this->header_id);
        ...
        if ($this->cekLevelUserNm == "Auditor") {
            $dtdetail   = $this->M_formfrmfss031_02->get_detail_byidx($this->header_id);
        } else {
            $dtdetail   = $this->M_formfrmfss031_02->get_detail_byid($this->header_id);
        }
        ...
        for ($i1 = 0; $i1 < $jml_page_a; $i1++) {
            ...
            $objPHPExcel->mergeCells('D' .  ($start_row + 2) . ':AH' . ($start_row + 2))->setCellValue('D' .  ($start_row + 2), $this->frmjdl);
            ...
            $objPHPExcel->mergeCells('B' .  ($hdr_row + 4) . ':E' .  ($hdr_row + 4))->setCellValue('B' .  ($hdr_row + 4), "Nama Mesin");
            ...
            $app_row = $dtl_row + 1;
            $objPHPExcel->mergeCells('B' . ($app_row) . ':N' .  ($app_row + 9))->setCellValue('B' .  ($app_row), "Hasil Pengujian/Running Test (coret yang tidak perlu) : \n" . $hasil_pengujian);
            ...
            $objPHPExcel->setSharedStyle($DetailheaderStyle, 'B' . ($app_row) . ':AN' . ($app_row + 6));

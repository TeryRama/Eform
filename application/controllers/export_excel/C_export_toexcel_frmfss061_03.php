...
class C_export_toexcel_frmfss061_03 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
    ...
    function exportxls()
    {
        ...
        $dtheader = $this->M_formfrmfss061_03->get_header_byid($this->header_id);
        ...
        $list_month = $this->M_formfrmfss061_03->get_month($tahun);
        if ($this->cekLevelUserNm == 'Auditor') {
            $dtdetail   = $this->M_formfrmfss061_03->get_detail_byidx($this->header_id);
        } else {
            $dtdetail   = $this->M_formfrmfss061_03->get_detail_byid($this->header_id);
        }
        ...
        if(isset($dtdetail)){
            ...
            foreach ($dtdetail as $dtdetail_row) { 
                ...
                $arr_jenis_oli[]    = $dtdetail_row->jenis_oli;
                $arr_volume[]       = $dtdetail_row->volume;
        ...
        $jml_row_perpage  = ($jml_data_perpage)+20;
        ...
        for ($i1 = 0; $i1 < $jml_page_a; $i1++) {
            ...
            $objPHPExcel->mergeCells('A' . ($start_row + 4) . ':AT' . ($start_row + 4))->setCellValue('A' . ($start_row + 4), ' GUGUS : ' . $gugus);
            ...
            $objPHPExcel->mergeCells('B' . ($start_row + 6) . ':H' . ($start_row + 7))->setCellValue('B' . ($start_row + 6), 'NAMA MESIN / ALAT');
            $objPHPExcel->mergeCells('I' . ($start_row + 6) . ':K' . ($start_row + 7))->setCellValue('I' . ($start_row + 6), 'KODE');
            $objPHPExcel->mergeCells('L' . ($start_row + 6) . ':P' . ($start_row + 7))->setCellValue('L' . ($start_row + 6), 'JENIS OLI/GREASE');
            $objPHPExcel->mergeCells('Q' . ($start_row + 6) . ':T' . ($start_row + 7))->setCellValue('Q' . ($start_row + 6), 'VOLUME (L/Kg)');
            $objPHPExcel->mergeCells('U' . ($start_row + 6) . ':W' . ($start_row + 7))->setCellValue('U' . ($start_row + 6), 'FREKWENSI (JAM)');
            
            $objPHPExcel->mergeCells('X' . ($start_row + 6) . ':AU' . ($start_row + 6))->setCellValue('X' . ($start_row + 6), "BULAN");

            $col_tgl = ['X','Z','AB','AD','AF','AH','AJ','AL','AN','AP','AR','AT'];
            $a1 = -1;
            foreach ($list_month as $list_month_row) {
                $a1++;
                $objPHPExcel->mergeCells(getColRange($col_tgl[$a1], ($start_row + 7), 1))->setCellValue($col_tgl[$a1] . ($start_row + 7), $bulan[$list_month_row->month]);
            }
            ...
            for ($arr = $start_detail; $arr <= $finish_detail; $arr++) {
                ...
                if(isset($arr_jenis_oli[$arr])){
                    $dt_jenis_oli[$arr] = $arr_jenis_oli[$arr];
                }else{
                    $dt_jenis_oli[$arr] = "";
                }
                if(isset($arr_volume[$arr])){
                    $dt_volume[$arr] = $arr_volume[$arr];
                }else{
                    $dt_volume[$arr] = "";
                }
                ...
                $objPHPExcel->mergeCells('L' . ($dtl_row) . ':P' . ($dtl_row))->setCellValue('L' . ($dtl_row), $dt_jenis_oli[$arr]);
                $objPHPExcel->mergeCells('Q' . ($dtl_row) . ':T' . ($dtl_row))->setCellValue('Q' . ($dtl_row), $dt_volume[$arr]);
                $objPHPExcel->mergeCells('U' . ($dtl_row) . ':W' . ($dtl_row))->setCellValue('U' . ($dtl_row), $dt_frekuensi[$arr]);
                
                $a1 = -1;
                foreach ($list_month as $list_month_row) {
                    $a1++;
                    $objPHPExcel->mergeCells(getColRange($col_tgl[$a1], ($dtl_row), 1))->setCellValue($col_tgl[$a1] . ($dtl_row), ${'dt_month'.$list_month_row->month}[$arr]);
                    
                    $arr_tgl_sch = explode(',', $dt_sch[$arr]);
                    foreach ($arr_tgl_sch as $arr_tgl_sch_row) {
                        if($arr_tgl_sch_row == $list_month_row->month){
                            $objPHPExcel->getStyle(getColRange($col_tgl[$a1], ($dtl_row), 1))->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => '4dffff'))));
                        }
                    }
                }
            ...
            $objPHPExcel->mergeCells('A' . ($app_row) . ':Q' . ($app_row))->setCellValue('A' . ($app_row), 'Dibuat Oleh,');
            $objPHPExcel->mergeCells('R' . ($app_row) . ':AI' . ($app_row))->setCellValue('R' . ($app_row), 'Diketahui Oleh,');
            $objPHPExcel->mergeCells('AJ' . ($app_row) . ':AZ' . ($app_row))->setCellValue('AJ' . ($app_row), 'Disetujui Oleh,');

            $objPHPExcel->mergeCells('A' . ($app_row + 1) . ':Q' . ($app_row + 3))->setCellValue('A' . ($app_row + 1), '');
            $objPHPExcel->mergeCells('R' . ($app_row + 1) . ':AI' . ($app_row + 3))->setCellValue('R' . ($app_row + 1), '');
            $objPHPExcel->mergeCells('AJ' . ($app_row + 1) . ':AZ' . ($app_row + 3))->setCellValue('AJ' . ($app_row + 1), '');

            $objPHPExcel->setSharedStyle($DetailheaderStyle, 'A' . ($app_row) . ':AZ' . ($app_row + 3));
            ...
            if (isset($app1_by)) {
                if (file_exists($fcpath2 . 'utl/assets/ttd/' . $app1_personalstatus . '_' . $app1_personalid . '.png')) {
                    ...
                    $sign_img->setCoordinates('I' . ($app_row + 1));
            ...
            if (isset($app2_by)) {
                if (file_exists($fcpath2 . 'utl/assets/ttd/' . $app2_personalstatus . '_' . $app2_personalid . '.png')) {
                    ...
                    $sign_img2->setCoordinates('Y' . ($app_row + 1));
                    ...
                    $sign_img->setResizeProportional(true);
                    $sign_img->setWorksheet($objPHPExcel);
                    $sign_img->setCoordinates('Y' . ($app_row + 1));
                }
            }
            ...
            if (isset($app3_by)) {
                if (file_exists($fcpath2 . 'assets/ttd/' . $app3_personalstatus . '_' . $app3_personalid . '.png')) {
                    ...
                    $sign_img3->setCoordinates('AR' . ($app_row + 1));
                    ...
                    $sign_img->setResizeProportional(true);
                    $sign_img->setWorksheet($objPHPExcel);
                    $sign_img->setCoordinates('AR' . ($app_row + 1));
                }
            }


            $objPHPExcel->mergeCells('A' . ($app_row + 4) . ':B' . ($app_row + 4))->setCellValue('A' . ($app_row + 4), 'Nama');
            $objPHPExcel->mergeCells('C' . ($app_row + 4) . ':Q' . ($app_row + 4))->setCellValue('C' . ($app_row + 4), ': ' . $app1_by);
            $objPHPExcel->mergeCells('A' . ($app_row + 5) . ':B' . ($app_row + 5))->setCellValue('A' . ($app_row + 5), 'Jabatan');
            $objPHPExcel->mergeCells('C' . ($app_row + 5) . ':Q' . ($app_row + 5))->setCellValue('C' . ($app_row + 5), ': ' . $app1_position);
            $objPHPExcel->mergeCells('A' . ($app_row + 6) . ':B' . ($app_row + 6))->setCellValue('A' . ($app_row + 6), 'Tanggal');
            $objPHPExcel->mergeCells('C' . ($app_row + 6) . ':Q' . ($app_row + 6))->setCellValue('C' . ($app_row + 6), ': ' . $app1date);

            $objPHPExcel->mergeCells('R' . ($app_row + 4) . ':T' . ($app_row + 4))->setCellValue('R' . ($app_row + 4), 'Nama');
            $objPHPExcel->mergeCells('U' . ($app_row + 4) . ':AI' . ($app_row + 4))->setCellValue('U' . ($app_row + 4), ': ' . $app2_by);
            $objPHPExcel->mergeCells('R' . ($app_row + 5) . ':T' . ($app_row + 5))->setCellValue('R' . ($app_row + 5), 'Jabatan');
            $objPHPExcel->mergeCells('U' . ($app_row + 5) . ':AI' . ($app_row + 5))->setCellValue('U' . ($app_row + 5), ': ' . $app2_position);
            $objPHPExcel->mergeCells('R' . ($app_row + 6) . ':T' . ($app_row + 6))->setCellValue('R' . ($app_row + 6), 'Tanggal');
            $objPHPExcel->mergeCells('U' . ($app_row + 6) . ':AI' . ($app_row + 6))->setCellValue('U' . ($app_row + 6), ': ' . $app2date);
            ...
            $objPHPExcel->mergeCells('AZ' . ($app_row + 2) . ':AZ' . ($app_row + 2))->setCellValue('AA' . ($app_row + 2), '');

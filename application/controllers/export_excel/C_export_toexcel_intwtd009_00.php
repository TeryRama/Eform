...
class C_export_toexcel_intwtd009_00 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
    ...
    function exportxls()
    {
        ...
        $dtheader = $this->M_formintwtd009_00->get_header_byid($this->header_id);
        ...
        $list_pj            = $this->M_forminput->get_list_pj($create_date, $this->frmnm);
        $dtkomponenmesin    = $this->M_formintwtd009_00->get_all_komponenmesin($this->BagianAkses);
        $dtjenis_mesin      = $this->M_formintwtd009_00->get_jenis_mesin_by($deptabbr, $create_date);
        $dtgugus            = $this->M_formintwtd009_00->get_gugus_by($deptabbr, $id_jns_mesin, $create_date);
        $dtitem_mesin       = $this->M_formintwtd009_00->get_itemmesin($deptabbr, $id_jns_mesin, $id_gugus, $create_date);
        if ($this->cekLevelUserNm == 'Auditor') {
            $dtdetail   = $this->M_formintwtd009_00->get_detail_byidx($this->header_id);
        } else {
            $dtdetail   = $this->M_formintwtd009_00->get_detail_byid($this->header_id);
        }

...
class C_export_toexcel_frmfss065_04 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
    ...
    function exportxls()
    {
        ...
        $dtheader = $this->M_formfrmfss065_04->get_header_byid($this->header_id);
        ...
        if ($this->cekLevelUserNm == 'Auditor') {
            $dtdetail   = $this->M_formfrmfss065_04->get_detail_byidx($this->header_id);
        } else {
            $dtdetail   = $this->M_formfrmfss065_04->get_detail_byid($this->header_id);
        }
        ...
        for ($i1 = 0; $i1 < $jml_page_a; $i1++) {
            ...
            $objPHPExcel->mergeCells('G' . ($foot_row + 2) . ':AZ' . ($foot_row + 2))->setCellValue('G' . ($foot_row + 2), 'Beri Tanda ✔ pada salah satu kolom Unplanned/Planned');

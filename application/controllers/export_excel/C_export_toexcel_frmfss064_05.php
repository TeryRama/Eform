...
class C_export_toexcel_frmfss064_05 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
    ...
    function exportxls()
    {
        ...
        $dtheader = $this->M_formfrmfss064_05->get_header_byid($this->header_id);
        ...
        if ($this->cekLevelUserNm == "Auditor") {
            $dtdetail   = $this->M_formfrmfss064_05->get_detail_byidx($this->header_id);
        } else {
            $dtdetail   = $this->M_formfrmfss064_05->get_detail_byid($this->header_id);
        }
        ...
        for ($i1 = 0; $i1 < $jml_page_a; $i1++) {
            ...
            $objPHPExcel->mergeCells('K' .  ($start_row + 4) . ':AN' .  ($start_row + 4))->setCellValue('K' .  ($start_row + 4), "SPESIFIKASI");
            $objPHPExcel->mergeCells('B' .  ($start_row + 4) . ':B' .  ($start_row + 6))->setCellValue('B' .  ($start_row + 4), "NO");
            $objPHPExcel->mergeCells('C' .  ($start_row + 4) . ':G' .  ($start_row + 6))->setCellValue('C' .  ($start_row + 4), "NAMA ITEM");
            $objPHPExcel->mergeCells('H' .  ($start_row + 4) . ':J' .  ($start_row + 6))->setCellValue('H' .  ($start_row + 4), "KODE ITEM");
            $objPHPExcel->mergeCells('K' .  ($start_row + 5) . ':M' .  ($start_row + 6))->setCellValue('K' .  ($start_row + 5), "MERK/JENIS");
            $objPHPExcel->mergeCells('N' .  ($start_row + 5) . ':P' .  ($start_row + 6))->setCellValue('N' .  ($start_row + 5), "TAHUN PERMBUATAN");
            $objPHPExcel->mergeCells('Q' .  ($start_row + 5) . ':S' .  ($start_row + 6))->setCellValue('Q' .  ($start_row + 5), "NO SERI");
            $objPHPExcel->mergeCells('T' .  ($start_row + 5) . ':V' .  ($start_row + 6))->setCellValue('T' .  ($start_row + 5), "DAYA (HP/kW)");
            $objPHPExcel->mergeCells('W' .  ($start_row + 5) . ':Y' .  ($start_row + 6))->setCellValue('W' .  ($start_row + 5), "PUTARAN (RPM)");
            $objPHPExcel->mergeCells('Z' .  ($start_row + 5) . ':AB' .  ($start_row + 6))->setCellValue('Z' .  ($start_row + 5), "TEGANGAN (VOLT)");
            $objPHPExcel->mergeCells('AC' .  ($start_row + 5) . ':AE' .  ($start_row + 6))->setCellValue('AC' .  ($start_row + 5), "ARUS (ampere)");
            $objPHPExcel->mergeCells('AF' .  ($start_row + 5) . ':AH' .  ($start_row + 6))->setCellValue('AF' .  ($start_row + 5), "BERAT (Kg)");
            $objPHPExcel->mergeCells('AI' .  ($start_row + 5) . ':AK' .  ($start_row + 6))->setCellValue('AI' .  ($start_row + 5), "KAPASITAS (TONASE)");
            $objPHPExcel->mergeCells('AL' .  ($start_row + 5) . ':AN' .  ($start_row + 6))->setCellValue('AL' .  ($start_row + 5), "Rangkain");
            $objPHPExcel->mergeCells('AO' .  ($start_row + 4) . ':AS' .  ($start_row + 6))->setCellValue('AO' .  ($start_row + 4), "KETERANGAN");

            $objPHPExcel->setSharedStyle($headerStyleRight, 'AT' . ($start_row + 3) . ':AT' . ($start_row + 6));
            $objPHPExcel->setSharedStyle($bodyStyleLeft,  'A' .  ($start_row + 3) . ':A' .  ($start_row + 6));
            $objPHPExcel->setSharedStyle($DetailheaderStyle, 'B' . ($start_row + 4) . ':AS' . ($start_row + 6));
            $objPHPExcel->getStyle('B' . ($start_row + 4) . ':AS' . ($start_row + 6))->getFont()->setBold(true)->setSize(9);
            ...
            for ($a = $start_detail; $a <= $finish_detail; $a++) {
                ...
                if ($dt_nama_mesin[$a] != '') {
                    ...
                    $objPHPExcel->mergeCells('C' .  ($dtl_row) . ':G' .  ($dtl_row))->setCellValue('C' .  ($dtl_row), $dt_nama_mesin[$a]);
                    $objPHPExcel->mergeCells('H' .  ($dtl_row) . ':J' .  ($dtl_row))->setCellValue('H' .  ($dtl_row), $dt_kode_mesin[$a]);
                    $objPHPExcel->mergeCells('K' .  ($dtl_row) . ':M' .  ($dtl_row))->setCellValue('K' .  ($dtl_row), $dt_merek[$a]);
                    $objPHPExcel->mergeCells('N' .  ($dtl_row) . ':P' .  ($dtl_row))->setCellValue('N' .  ($dtl_row), $dt_tahun[$a]);
                    $objPHPExcel->mergeCells('Q' .  ($dtl_row) . ':S' .  ($dtl_row))->setCellValue('Q' .  ($dtl_row), $dt_seri[$a]);
                    $objPHPExcel->mergeCells('T' .  ($dtl_row) . ':V' .  ($dtl_row))->setCellValue('T' .  ($dtl_row), $dt_daya[$a]);
                    $objPHPExcel->mergeCells('W' .  ($dtl_row) . ':Y' .  ($dtl_row))->setCellValue('W' .  ($dtl_row), $dt_putaran[$a]);
                    $objPHPExcel->mergeCells('Z' .  ($dtl_row) . ':AB' .  ($dtl_row))->setCellValue('Z' .  ($dtl_row), $dt_tegangan[$a]);
                    $objPHPExcel->mergeCells('AC' .  ($dtl_row) . ':AE' .  ($dtl_row))->setCellValue('AC' .  ($dtl_row), $dt_arus[$a]);
                    $objPHPExcel->mergeCells('AF' .  ($dtl_row) . ':AH' .  ($dtl_row))->setCellValue('AF' .  ($dtl_row), $dt_berat[$a]);
                    $objPHPExcel->mergeCells('AI' .  ($dtl_row) . ':AK' .  ($dtl_row))->setCellValue('AI' .  ($dtl_row), $dt_kapasitas[$a]);
                    $objPHPExcel->mergeCells('AL' .  ($dtl_row) . ':AN' .  ($dtl_row))->setCellValue('AL' .  ($dtl_row), $dt_rangkain[$a]);
                    $objPHPExcel->mergeCells('AO' .  ($dtl_row) . ':AS' .  ($dtl_row))->setCellValue('AO' .  ($dtl_row), $dt_ket[$a]);
                } else {
                    $objPHPExcel->mergeCells('B' .  ($dtl_row) . ':B' .  ($dtl_row))->setCellValue('B' .  ($dtl_row), "");
                    $objPHPExcel->mergeCells('C' .  ($dtl_row) . ':G' .  ($dtl_row))->setCellValue('C' .  ($dtl_row), "");
                    ...
                    $objPHPExcel->mergeCells('T' .  ($dtl_row) . ':V' .  ($dtl_row))->setCellValue('T' .  ($dtl_row), "");
                    $objPHPExcel->mergeCells('W' .  ($dtl_row) . ':Y' .  ($dtl_row))->setCellValue('W' .  ($dtl_row), "");
                    $objPHPExcel->mergeCells('Z' .  ($dtl_row) . ':AB' .  ($dtl_row))->setCellValue('Z' .  ($dtl_row), "");
                    $objPHPExcel->mergeCells('AC' .  ($dtl_row) . ':AE' .  ($dtl_row))->setCellValue('AC' .  ($dtl_row), "");
                    $objPHPExcel->mergeCells('AF' .  ($dtl_row) . ':AH' .  ($dtl_row))->setCellValue('AF' .  ($dtl_row), "");
                    $objPHPExcel->mergeCells('AI' .  ($dtl_row) . ':AK' .  ($dtl_row))->setCellValue('AI' .  ($dtl_row), "");
                    $objPHPExcel->mergeCells('AL' .  ($dtl_row) . ':AN' .  ($dtl_row))->setCellValue('AL' .  ($dtl_row), "");
            ...
            $objPHPExcel->mergeCells('AD' . ($app_row) . ':AS' . ($app_row + 1))->setCellValue('AD' . ($app_row), 'Disetujui Oleh,');
            ...
            $objPHPExcel->setSharedStyle($DetailheaderStyle, 'B' . ($app_row) . ':AS' . ($app_row + 6));
            ...
            $objPHPExcel->mergeCells('AG' . ($app_row + 7) . ':AS' . ($app_row + 7))->setCellValue('AG' . ($app_row + 7), ': ' . $app3_by);
            $objPHPExcel->mergeCells('AD' . ($app_row + 8) . ':AF' . ($app_row + 8))->setCellValue('AD' . ($app_row + 8), 'Jabatan');
            $objPHPExcel->mergeCells('AG' . ($app_row + 8) . ':AS' . ($app_row + 8))->setCellValue('AG' . ($app_row + 8), ': ' . $app3_position);
            $objPHPExcel->mergeCells('AD' . ($app_row + 9) . ':AF' . ($app_row + 9))->setCellValue('AD' . ($app_row + 9), 'Tanggal');
            $objPHPExcel->mergeCells('AG' . ($app_row + 9) . ':AS' . ($app_row + 9))->setCellValue('AG' . ($app_row + 9), ': ' . $app3date);

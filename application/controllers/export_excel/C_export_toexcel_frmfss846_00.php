...
class C_export_toexcel_frmfss846_00 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
    ...
    function exportxls()
    {
        ...
        $dtheader = $this->M_formfrmfss846_00->get_header_byid($this->header_id);
        ...
        $list_pj            = $this->M_forminput->get_list_pj($dtcreate_date, $frmnm);
        $dtkomponenmesin    = $this->model->get_all_komponenmesin($BagianAkses);
        $dtjenis_mesin      = $this->model->get_jenis_mesin_by($dtdept, $dtcreate_date);
        $dtkode             = $this->model->get_kode_by($dept, $id_nama_panel, $create_date);
        $dtlokasi           = $this->model->get_lokasi_by($dept, $id_nama_panel, $create_date);
        $dtitem_mesin       = $this->model->get_itemmesin($dept, $id_nama_panel, $id_kode_panel, $lokasi_panel);
        if ($this->cekLevelUserNm == 'Auditor') {
            $dtdetail   = $this->M_formfrmfss846_00->get_detail_byidx($this->header_id);
        } else {
            $dtdetail   = $this->M_formfrmfss846_00->get_detail_byid($this->header_id);
        }
        ...
        if(isset($dtdetail)){
            ...
            foreach ($dtdetail as $dtdetail_row) {
                $arr_number[]           = $no++;
                $arr_waktu_tanggal[]    = trim ($dtdetail_row->waktu_tanggal);
                $arr_waktu_jam[]        = trim ($dtdetail_row->waktu_jam);
                $arr_kabel_induk_a[]    = trim ($dtdetail_row->kabel_induk_a);
                $arr_kabel_induk_c[]    = trim ($dtdetail_row->kabel_induk_c);
                $arr_busbar[]           = trim ($dtdetail_row->busbar);
                $arr_ampere_meter[]     = trim ($dtdetail_row->ampere_meter);
                $arr_volt_meter[]       = trim ($dtdetail_row->volt_meter);
                $arr_kebersihan[]       = trim ($dtdetail_row->kebersihan);
                $arr_ket[]              = trim ($dtdetail_row->ket);

                for ($i_komponen=1; $i_komponen <= count($dtkomponenmesin); $i_komponen++) {
                    ${'arr_kabel_induk_a'.$i_komponen}[] = $dtdetail_row->{'kabel_induk_a'.$i_komponen};
                    ${'arr_kabel_induk_c'.$i_komponen}[] = $dtdetail_row->{'kabel_induk_c'.$i_komponen};
                }
            } 
        }
        ...
        for ($i1 = 0; $i1 < $jml_page_a; $i1++) {
            ...
            for ($arr = $start_detail; $arr <= $finish_detail; $arr++) {
                ...
                if(isset($arr_waktu_tanggal[$arr])){
                    $dt_waktu_tanggal[$arr] = $arr_waktu_tanggal[$arr];
                }else{
                    $dt_waktu_tanggal[$arr] = "";
                }
                if(isset($arr_waktu_jam[$arr])){
                    $dt_waktu_jam[$arr] = $arr_waktu_jam[$arr];
                }else{
                    $dt_waktu_jam[$arr] = "";
                }
                if(isset($arr_kabel_induk_a[$arr])){
                    $dt_kabel_induk_a[$arr] = $arr_kabel_induk_a[$arr];
                }else{
                    $dt_kabel_induk_a[$arr] = "";
                }
                if(isset($arr_kabel_induk_c[$arr])){
                    $dt_kabel_induk_c[$arr] = $arr_kabel_induk_c[$arr];
                }else{
                    $dt_kabel_induk_c[$arr] = "";
                }
                if(isset($arr_busbar[$arr])){
                    $dt_busbar[$arr] = $arr_busbar[$arr];
                }else{
                    $dt_busbar[$arr] = "";
                }
                if(isset($arr_ampere_meter[$arr])){
                    $dt_ampere_meter[$arr] = $arr_ampere_meter[$arr];
                }else{
                    $dt_ampere_meter[$arr] = "";
                }
                if(isset($arr_volt_meter[$arr])){
                    $dt_volt_meter[$arr] = $arr_volt_meter[$arr];
                }else{
                    $dt_volt_meter[$arr] = "";
                }
                if(isset($arr_kebersihan[$arr])){
                    $dt_kebersihan[$arr] = $arr_kebersihan[$arr];
                }else{
                    $dt_kebersihan[$arr] = "";
                }
                ...
                for ($i_komponen=1; $i_komponen <= count($dtkomponenmesin); $i_komponen++) {
                    if(isset(${'arr_kabel_induk_a'.$i_komponen}[$arr])){
                        ${'dt_kabel_induk_a'.$i_komponen}[$arr] = ${'arr_kabel_induk_a'.$i_komponen}[$arr];
                    }else{
                        ${'dt_kabel_induk_a'.$i_komponen}[$arr] = "";
                    }
                    if(isset(${'arr_kabel_induk_c'.$i_komponen}[$arr])){
                        ${'dt_kabel_induk_c'.$i_komponen}[$arr] = ${'arr_kabel_induk_c'.$i_komponen}[$arr];
                    }else{
                        ${'dt_kabel_induk_c'.$i_komponen}[$arr] = "";
                    }
                }

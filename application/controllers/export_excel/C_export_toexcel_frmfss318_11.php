...
class C_export_toexcel_frmfss318_11 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
        ...
        for ($i1 = 0; $i1 < $jml_page; $i1++) {
            ...
            for ($a = $start_detail; $a <= $finish_detail; $a++) {
                ...
                if ($no_urut_mesin[$a] == 1) {
                    if($dt_nama_mesin[$a] == 'Flowrate air'){
                    ...
                    }else if($dt_nama_mesin[$a] == 'Softener'){
                        $objPHPExcel->mergeCells('B' . $dtl_row . ':C' . ($dtl_row + (($jml_per_mesin[$a]+7)*$jml_per_kode[$a]-1)))->setCellValue('B' . $dtl_row, $no++);
                        $objPHPExcel->mergeCells('D' .  $dtl_row . ':F' .  ($dtl_row + (($jml_per_mesin[$a]+7)*$jml_per_kode[$a]-1)))->setCellValue('D' .  $dtl_row, $dt_nama_mesin[$a]);
                    }else{
                        $objPHPExcel->mergeCells('B' . $dtl_row . ':C' . ($dtl_row + (($jml_per_mesin[$a]+12)*$jml_per_kode[$a]-1)))->setCellValue('B' . $dtl_row, $no++);
                        $objPHPExcel->mergeCells('D' .  $dtl_row . ':F' .  ($dtl_row + (($jml_per_mesin[$a]+12)*$jml_per_kode[$a]-1)))->setCellValue('D' .  $dtl_row, $dt_nama_mesin[$a]);
                    }
                } else {

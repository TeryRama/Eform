...
class C_export_toexcel_frmfss520_09 extends CI_Controller
{
    private $xls;
    private $spreadsheet;
    ...
    function exportxls()
    {
        ...
        if ($this->cekLevelUserNm == "Auditor") {
            $dtdetail       = $this->model->get_detail_lap_byidx($this->header_id);
            $dtdetail_b     = $this->model->get_detail_byid_lapbx($this->header_id);
            $dtdetail_c     = $this->model->get_detail_byid_lapcx($this->header_id);
            $dtdetail_d     = $this->model->get_detail_byid_lapdx($this->header_id);
        } else {
            $dtdetail       = $this->model->get_detail_lap_byid($this->header_id);
            $dtdetail_b     = $this->model->get_detail_byid_lapb($this->header_id);
            $dtdetail_c     = $this->model->get_detail_byid_lapc($this->header_id);
        ...
        foreach ($dtdetail as $dtdetail_key => $dtdetail_row) {
            ...
            if ($dtdetail_row->shift == 'shift_1') {
                ...
                // if ($no <= 2) {
                //     $vitem1 = '<td align="center" rowspan="' . $dtdetail_row->jml_per_mesin . '"><b>' . $dtl_nama_mesin . '</b></td>'; // gabungin jadi satu kolom dan tulisan miring per nama mesin
                // } else {
                //     $vitem1 = '';
                // }
        ...
        for ($tbl_b_i = 0; $tbl_b_i < $tbl_b_max; $tbl_b_i++) {
            for ($tbl_b_i2 = 1; $tbl_b_i2 <= 3; $tbl_b_i2++) {
                ${'time_check_shift_' . $tbl_b_i2}[]   = !empty($dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]) ? $dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]->time_check : '';
                ${'ph_air_shift_' . $tbl_b_i2}[]       = !empty($dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]) ? $dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]->ph_air : '';
                ${'ph_caustic_shift_' . $tbl_b_i2}[]   = !empty($dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]) ? $dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]->ph_caustic : '';
                ${'temp_caustic_shift_' . $tbl_b_i2}[] = !empty($dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]) ? $dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]->temp_caustic : '';
                ${'ph_acid_shift_' . $tbl_b_i2}[]      = !empty($dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]) ? $dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]->ph_acid : '';
                ${'temp_acid_shift_' . $tbl_b_i2}[]    = !empty($dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]) ? $dtdetail_b['shift_' . $tbl_b_i2][$tbl_b_i]->temp_acid : '';
            }
            $no_b++;
        }

        // detail c
        $tbl_c_max = max(count($dtdetail_c['shift_1']), count($dtdetail_c['shift_2']), count($dtdetail_c['shift_3']));
        $no_c = 1;
        for ($tbl_c_i = 0; $tbl_c_i < $tbl_c_max; $tbl_c_i++) {
            for ($tbl_c_i2 = 1; $tbl_c_i2 <= 3; $tbl_c_i2++) {
                ${'time_check2_shift_' . $tbl_c_i2}[]    = !empty($dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]) ? $dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]->time_check : '';
                ${'ph_shift_' . $tbl_c_i2}[]             = !empty($dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]) ? $dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]->ph : '';
                ${'residu_caustic_shift_' . $tbl_c_i2}[] = !empty($dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]) ? $dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]->residu_caustic : '';
                ${'residu_acid_shift_' . $tbl_c_i2}[]    = !empty($dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]) ? $dtdetail_c['shift_' . $tbl_c_i2][$tbl_c_i]->residu_acid : '';
            }
            $no_c++;
        }
        ...
        foreach ($dtdetail_d as $dtdetail_d_row) {
            $nomor[]                    = $no++;
            $dtl_d_shift[]              = trim ($dtdetail_d_row->shift);
            $dtl_d_jam[]                = trim ($dtdetail_d_row->jam);
            $dtl_d_uraian[]             = trim ($dtdetail_d_row->uraian);
            $dtl_d_tindakan[]           = trim ($dtdetail_d_row->tindakan);
            $dtl_d_pj_nama[]            = trim ($dtdetail_d_row->pj_nama);
            $dtl_d_pj_personalstatus[]  = trim ($dtdetail_d_row->pj_personalstatus);
            $dtl_d_pj_personalid[]      = trim ($dtdetail_d_row->pj_personalid);
            $dtl_d_paraf[]              = trim ($dtdetail_d_row->paraf);
            $dtl_d_keterangan[]         = trim ($dtdetail_d_row->keterangan);
        ...
        for ($i1 = 0; $i1 < $jml_page; $i1++) {
            ...
            for ($a = 0; $a <= $bagidata[$i1]; $a++) {
                $dtl_row++;
                $jumlahdata++;

                $dt_no[$jumlahdata-1]         = $dtl_no[$jumlahdata-1]         ?? "";
                $dt_nama_mesin[$jumlahdata-1] = $dtl_nama_mesin[$jumlahdata-1] ?? "";
                $dt_kode[$jumlahdata-1]       = $dtl_kode[$jumlahdata-1]       ?? "";
                $dt_parameter[$jumlahdata-1]  = $dtl_parameter[$jumlahdata-1]  ?? "";

                $dt_cek_shift_jam1_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam1_shift_1[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam2_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam2_shift_1[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam3_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam3_shift_1[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam4_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam4_shift_1[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam5_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam5_shift_1[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam6_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam6_shift_1[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam7_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam7_shift_1[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam8_shift_1[$jumlahdata-1] = $dtl_cek_shift_jam8_shift_1[$jumlahdata-1] ?? "";

                $dt_cek_shift_jam1_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam1_shift_2[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam2_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam2_shift_2[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam3_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam3_shift_2[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam4_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam4_shift_2[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam5_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam5_shift_2[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam6_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam6_shift_2[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam7_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam7_shift_2[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam8_shift_2[$jumlahdata-1] = $dtl_cek_shift_jam8_shift_2[$jumlahdata-1] ?? "";

                $dt_cek_shift_jam1_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam1_shift_3[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam2_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam2_shift_3[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam3_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam3_shift_3[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam4_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam4_shift_3[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam5_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam5_shift_3[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam6_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam6_shift_3[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam7_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam7_shift_3[$jumlahdata-1] ?? "";
                $dt_cek_shift_jam8_shift_3[$jumlahdata-1] = $dtl_cek_shift_jam8_shift_3[$jumlahdata-1] ?? "";

                if (isset($no_urut_desc[$jumlahdata-1])) {
                    $dt_no_urut_desc[$jumlahdata-1]      = $no_urut_desc[$jumlahdata-1] - 1;
                } else {
                    $dt_no_urut_desc[$jumlahdata-1]    = 0;
                }
                if (isset($no_urut[$jumlahdata-1])) {
                    $dt_no_urut[$jumlahdata-1]                  = $no_urut[$jumlahdata-1];
                } else {
                    $dt_no_urut[$jumlahdata-1]    = "";
                }
                // if (isset($nama_mesin[$jumlahdata-1])) {
                //     $dt_nama_mesin[$jumlahdata-1]    = $nama_mesin[$jumlahdata-1];
                // } else {
                //     $dt_nama_mesin[$jumlahdata-1]    = "";
                // }

                if ($dt_no_urut[$jumlahdata-1] == 1) {
                    $objPHPExcel->mergeCells('D' .  $dtl_row . ':F' .  ($dtl_row + $dt_no_urut_desc[$jumlahdata-1]))->setCellValue('D' .  $dtl_row, $dt_nama_mesin[$jumlahdata-1]);
                    $objPHPExcel->mergeCells('BP' . $dtl_row  . ':BV' . ($dtl_row + $dt_no_urut_desc[$jumlahdata-1]))->setCellValue('BP' . $dtl_row, "PG : Pressure Gauge
                                                                                                                                                        DP : Dosing Pump Dosing Pump;
                                                                                                                                                        FM : Flow Meter  
                                                                                                                                                        M3 : Meter kubik

                                                                                                                                                        CIP dilakukan jika :
                                                                                                                                                        a. Product turun 10-15% performa awal
                                                                                                                                                        b. Reject naik 5-10% performa awal
                                                                                                                                                        c. ∆P Pressure stage 1 dan stage 2 dan recovery pressure naik 30% dari performa awal
                                                                                                                                                        d. Maximum selisih pressure stage 2 dan recovery pressure  = 10  Bar, Max");
                } else {
                ...
                $objPHPExcel->setSharedStyle($bodyStyle, 'B' . $dtl_row . ':BV' . $dtl_row);
                $objPHPExcel->mergeCells('B' . $dtl_row . ':C' . $dtl_row)->setCellValue('B' . $dtl_row, $dt_no[$jumlahdata-1]);
                // $objPHPExcel->mergeCells('D' . $dtl_row . ':F' . $dtl_row)->setCellValue('D' . $dtl_row, $dt_nama_mesin[$jumlahdata-1]);
                $objPHPExcel->mergeCells('G' . $dtl_row . ':K' . $dtl_row)->setCellValue('G' . $dtl_row, $dt_kode[$jumlahdata-1]);
                $objPHPExcel->mergeCells('L' . $dtl_row . ':S' . $dtl_row)->setCellValue('L' . $dtl_row, $dt_parameter[$jumlahdata-1]);

                $objPHPExcel->mergeCells('T' . $dtl_row . ':U' . $dtl_row)->setCellValue('T' . $dtl_row, $dt_cek_shift_jam1_shift_1[$jumlahdata-1]);
                $objPHPExcel->mergeCells('V' . $dtl_row . ':W' . $dtl_row)->setCellValue('V' . $dtl_row, $dt_cek_shift_jam2_shift_1[$jumlahdata-1]);
                $objPHPExcel->mergeCells('X' . $dtl_row . ':Y' . $dtl_row)->setCellValue('X' . $dtl_row, $dt_cek_shift_jam3_shift_1[$jumlahdata-1]);
                $objPHPExcel->mergeCells('Z' . $dtl_row . ':AA' . $dtl_row)->setCellValue('Z' . $dtl_row, $dt_cek_shift_jam4_shift_1[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AB' . $dtl_row . ':AC' . $dtl_row)->setCellValue('AB' . $dtl_row, $dt_cek_shift_jam5_shift_1[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AD' . $dtl_row . ':AE' . $dtl_row)->setCellValue('AD' . $dtl_row, $dt_cek_shift_jam6_shift_1[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AF' . $dtl_row . ':AG' . $dtl_row)->setCellValue('AF' . $dtl_row, $dt_cek_shift_jam7_shift_1[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AH' . $dtl_row . ':AI' . $dtl_row)->setCellValue('AH' . $dtl_row, $dt_cek_shift_jam8_shift_1[$jumlahdata-1]);
                
                $objPHPExcel->mergeCells('AJ' . $dtl_row . ':AK' . $dtl_row)->setCellValue('AJ' . $dtl_row, $dt_cek_shift_jam1_shift_2[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AL' . $dtl_row . ':AM' . $dtl_row)->setCellValue('AL' . $dtl_row, $dt_cek_shift_jam2_shift_2[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AN' . $dtl_row . ':AO' . $dtl_row)->setCellValue('AN' . $dtl_row, $dt_cek_shift_jam3_shift_2[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AP' . $dtl_row . ':AQ' . $dtl_row)->setCellValue('AP' . $dtl_row, $dt_cek_shift_jam4_shift_2[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AR' . $dtl_row . ':AS' . $dtl_row)->setCellValue('AR' . $dtl_row, $dt_cek_shift_jam5_shift_2[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AT' . $dtl_row . ':AU' . $dtl_row)->setCellValue('AT' . $dtl_row, $dt_cek_shift_jam6_shift_2[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AV' . $dtl_row . ':AW' . $dtl_row)->setCellValue('AV' . $dtl_row, $dt_cek_shift_jam7_shift_2[$jumlahdata-1]);
                $objPHPExcel->mergeCells('AX' . $dtl_row . ':AY' . $dtl_row)->setCellValue('AX' . $dtl_row, $dt_cek_shift_jam8_shift_2[$jumlahdata-1]);

                $objPHPExcel->mergeCells('AZ' . $dtl_row . ':BA' . $dtl_row)->setCellValue('AZ' . $dtl_row, $dt_cek_shift_jam1_shift_3[$jumlahdata-1]);
                $objPHPExcel->mergeCells('BB' . $dtl_row . ':BC' . $dtl_row)->setCellValue('BB' . $dtl_row, $dt_cek_shift_jam2_shift_3[$jumlahdata-1]);
                $objPHPExcel->mergeCells('BD' . $dtl_row . ':BE' . $dtl_row)->setCellValue('BD' . $dtl_row, $dt_cek_shift_jam3_shift_3[$jumlahdata-1]);
                $objPHPExcel->mergeCells('BF' . $dtl_row . ':BG' . $dtl_row)->setCellValue('BF' . $dtl_row, $dt_cek_shift_jam4_shift_3[$jumlahdata-1]);
                $objPHPExcel->mergeCells('BH' . $dtl_row . ':BI' . $dtl_row)->setCellValue('BH' . $dtl_row, $dt_cek_shift_jam5_shift_3[$jumlahdata-1]);
                $objPHPExcel->mergeCells('BJ' . $dtl_row . ':BK' . $dtl_row)->setCellValue('BJ' . $dtl_row, $dt_cek_shift_jam6_shift_3[$jumlahdata-1]);
                $objPHPExcel->mergeCells('BL' . $dtl_row . ':BM' . $dtl_row)->setCellValue('BL' . $dtl_row, $dt_cek_shift_jam7_shift_3[$jumlahdata-1]);
                $objPHPExcel->mergeCells('BN' . $dtl_row . ':BO' . $dtl_row)->setCellValue('BN' . $dtl_row, $dt_cek_shift_jam8_shift_3[$jumlahdata-1]);
                ...
                $objPHPExcel->setSharedStyle($bodyStyleAlignLeft, 'BP' . ($dtl_row) . ':BV' . ($dtl_row + 9));
            ...
            if (($i1 + 1) != ($jml_page)) {
                $app_row = $dtl_row;
                $objPHPExcel->setSharedStyle($bodyStyleLeft, 'A' . ($app_row + 1) . ':A' . ($app_row + 9));
                $objPHPExcel->setSharedStyle($noborderStyle, 'B' . ($app_row + 1) . ':BV' . ($app_row + 9));
                $objPHPExcel->setSharedStyle($bodyStyleRight, 'BW' . ($app_row + 1) . ':BW' . ($app_row + 9));
            }else{
                $start_table2 = $dtl_row;
                // detail b
                $objPHPExcel->getRowDimension($start_table2 + 4)->setRowHeight(25);
                $objPHPExcel->mergeCells('B'  . ($start_table2 + 1) . ':BV' . ($start_table2 + 1))->setCellValue('B' .  ($start_table2 + 1), "Hasil Analisa Sebelum CIP ");

                $objPHPExcel->mergeCells('B'  . ($start_table2 + 2) . ':Z' . ($start_table2 + 2))->setCellValue('B' .  ($start_table2 + 2), "Shift I");
                $objPHPExcel->mergeCells('B'  . ($start_table2 + 3) . ':F' . ($start_table2 + 3))->setCellValue('B' .  ($start_table2 + 3), "Time Check ");
                $objPHPExcel->mergeCells('G'  . ($start_table2 + 3) . ':I' . ($start_table2 + 3))->setCellValue('G' .  ($start_table2 + 3), "pH air ");
                $objPHPExcel->mergeCells('J'  . ($start_table2 + 3) . ':P' . ($start_table2 + 3))->setCellValue('J' .  ($start_table2 + 3), "Setelah Caustic");
                $objPHPExcel->mergeCells('Q'  . ($start_table2 + 3) . ':Z' . ($start_table2 + 3))->setCellValue('Q' .  ($start_table2 + 3), "Setelah Acid");

                $objPHPExcel->mergeCells('B'  . ($start_table2 + 4) . ':F' . ($start_table2 + 4))->setCellValue('B' .  ($start_table2 + 4), "CIP ");
                $objPHPExcel->mergeCells('G'  . ($start_table2 + 4) . ':I' . ($start_table2 + 4))->setCellValue('G' .  ($start_table2 + 4), "(6,5-8,5)");
                $objPHPExcel->mergeCells('J'  . ($start_table2 + 4) . ':M' . ($start_table2 + 4))->setCellValue('J' .  ($start_table2 + 4), "pH\n10,5-12,0)");
                $objPHPExcel->mergeCells('N'  . ($start_table2 + 4) . ':P' . ($start_table2 + 4))->setCellValue('N' .  ($start_table2 + 4), "Temp\n(40-45'C)");
                $objPHPExcel->mergeCells('Q'  . ($start_table2 + 4) . ':T' . ($start_table2 + 4))->setCellValue('Q' .  ($start_table2 + 4), "pH (3,0-5,0)");
                $objPHPExcel->mergeCells('U'  . ($start_table2 + 4) . ':Z' . ($start_table2 + 4))->setCellValue('U' .  ($start_table2 + 4), "Temp (40-45'C)");

                $objPHPExcel->mergeCells('AA'  . ($start_table2 + 2) . ':AW' . ($start_table2 + 2))->setCellValue('AA' .  ($start_table2 + 2), "Shift 2");
                $objPHPExcel->mergeCells('AA'  . ($start_table2 + 3) . ':AC' . ($start_table2 + 3))->setCellValue('AA' .  ($start_table2 + 3), "Time Check ");
                $objPHPExcel->mergeCells('AD'  . ($start_table2 + 3) . ':AF' . ($start_table2 + 3))->setCellValue('AD' .  ($start_table2 + 3), "pH air ");
                $objPHPExcel->mergeCells('AG'  . ($start_table2 + 3) . ':AM' . ($start_table2 + 3))->setCellValue('AG' .  ($start_table2 + 3), "Setelah Caustic");
                $objPHPExcel->mergeCells('AN'  . ($start_table2 + 3) . ':AW' . ($start_table2 + 3))->setCellValue('AN' .  ($start_table2 + 3), "Setelah Acid");

                $objPHPExcel->mergeCells('AA'  . ($start_table2 + 4) . ':AC' . ($start_table2 + 4))->setCellValue('AA' .  ($start_table2 + 4), "CIP ");
                $objPHPExcel->mergeCells('AD'  . ($start_table2 + 4) . ':AF' . ($start_table2 + 4))->setCellValue('AD' .  ($start_table2 + 4), "(6,5-8,5)");
                $objPHPExcel->mergeCells('AG'  . ($start_table2 + 4) . ':AJ' . ($start_table2 + 4))->setCellValue('AG' .  ($start_table2 + 4), "pH\n(10,5-12,0)");
                $objPHPExcel->mergeCells('AK'  . ($start_table2 + 4) . ':AM' . ($start_table2 + 4))->setCellValue('AK' .  ($start_table2 + 4), "Temp\n(40-45'C)");
                $objPHPExcel->mergeCells('AN'  . ($start_table2 + 4) . ':AQ' . ($start_table2 + 4))->setCellValue('AN' .  ($start_table2 + 4), "pH (3,0-5,0)");
                $objPHPExcel->mergeCells('AR'  . ($start_table2 + 4) . ':AW' . ($start_table2 + 4))->setCellValue('AR' .  ($start_table2 + 4), "Temp (40-45'C)");

                $objPHPExcel->mergeCells('AX'  . ($start_table2 + 2) . ':BV' . ($start_table2 + 2))->setCellValue('AX' .  ($start_table2 + 2), "Shift 3");
                $objPHPExcel->mergeCells('AX'  . ($start_table2 + 3) . ':AZ' . ($start_table2 + 3))->setCellValue('AX' .  ($start_table2 + 3), "Time Check ");
                $objPHPExcel->mergeCells('BA'  . ($start_table2 + 3) . ':BC' . ($start_table2 + 3))->setCellValue('BA' .  ($start_table2 + 3), "pH air ");
                $objPHPExcel->mergeCells('BD'  . ($start_table2 + 3) . ':BJ' . ($start_table2 + 3))->setCellValue('BD' .  ($start_table2 + 3), "Setelah Caustic");
                $objPHPExcel->mergeCells('BK'  . ($start_table2 + 3) . ':BV' . ($start_table2 + 3))->setCellValue('BK' .  ($start_table2 + 3), "Setelah Acid");

                $objPHPExcel->mergeCells('AX'  . ($start_table2 + 4) . ':AZ' . ($start_table2 + 4))->setCellValue('AX' .  ($start_table2 + 4), "CIP ");
                $objPHPExcel->mergeCells('BA'  . ($start_table2 + 4) . ':BC' . ($start_table2 + 4))->setCellValue('BA' .  ($start_table2 + 4), "(6,5-8,5)");
                $objPHPExcel->mergeCells('BD'  . ($start_table2 + 4) . ':BG' . ($start_table2 + 4))->setCellValue('BD' .  ($start_table2 + 4), "pH\n(10,5-12,0)");
                $objPHPExcel->mergeCells('BH'  . ($start_table2 + 4) . ':BJ' . ($start_table2 + 4))->setCellValue('BH' .  ($start_table2 + 4), "Temp\n(40-45'C)");
                $objPHPExcel->mergeCells('BK'  . ($start_table2 + 4) . ':BN' . ($start_table2 + 4))->setCellValue('BK' .  ($start_table2 + 4), "pH (3,0-5,0)");
                $objPHPExcel->mergeCells('BO'  . ($start_table2 + 4) . ':BV' . ($start_table2 + 4))->setCellValue('BO' .  ($start_table2 + 4), "Temp (40-45'C)");

                $objPHPExcel->setSharedStyle($noborderStyle, 'B' . ($start_table2 + 1) . ':BV' . ($start_table2 + 4));
                $objPHPExcel->setSharedStyle($bodyStyleRight, 'BW' . ($start_table2 + 1) . ':BW' . ($start_table2 + 4));
                $objPHPExcel->setSharedStyle($bodyStyleLeft, 'A' . ($start_table2 + 1) . ':A' . ($start_table2 + 4));
                $objPHPExcel->setSharedStyle($DetailheaderStyle, 'B' . ($start_table2 + 1) . ':BV' . ($start_table2 + 4));
                $objPHPExcel->getStyle('B' . ($start_table2 + 4) . ':BV' . ($start_table2 + 4))->getFont()->setBold(true);
            
                $dtl_row2 = $start_table2 + 4;
                for ($b = 0; $b <= 0; $b++) {
                    $dtl_row2++;
                    $objPHPExcel->getRowDimension($dtl_row2)->setRowHeight(25);
                    $dt_time_check_shift_1  [$b] = $time_check_shift_1  [$b] ?? "";
                    $dt_ph_air_shift_1      [$b] = $ph_air_shift_1      [$b] ?? "";
                    $dt_ph_caustic_shift_1  [$b] = $ph_caustic_shift_1  [$b] ?? "";
                    $dt_temp_caustic_shift_1[$b] = $temp_caustic_shift_1[$b] ?? "";
                    $dt_ph_acid_shift_1     [$b] = $ph_acid_shift_1     [$b] ?? "";
                    $dt_temp_acid_shift_1   [$b] = $temp_acid_shift_1   [$b] ?? "";

                    $dt_time_check_shift_2  [$b] = $time_check_shift_2  [$b] ?? "";
                    $dt_ph_air_shift_2      [$b] = $ph_air_shift_2      [$b] ?? "";
                    $dt_ph_caustic_shift_2  [$b] = $ph_caustic_shift_2  [$b] ?? "";
                    $dt_temp_caustic_shift_2[$b] = $temp_caustic_shift_2[$b] ?? "";
                    $dt_ph_acid_shift_2     [$b] = $ph_acid_shift_2     [$b] ?? "";
                    $dt_temp_acid_shift_2   [$b] = $temp_acid_shift_2   [$b] ?? "";

                    $dt_time_check_shift_3  [$b] = $time_check_shift_3  [$b] ?? "";
                    $dt_ph_air_shift_3      [$b] = $ph_air_shift_3      [$b] ?? "";
                    $dt_ph_caustic_shift_3  [$b] = $ph_caustic_shift_3  [$b] ?? "";
                    $dt_temp_caustic_shift_3[$b] = $temp_caustic_shift_3[$b] ?? "";
                    $dt_ph_acid_shift_3     [$b] = $ph_acid_shift_3     [$b] ?? "";
                    $dt_temp_acid_shift_3   [$b] = $temp_acid_shift_3   [$b] ?? "";


                    $objPHPExcel->setSharedStyle($noborderStyle, 'B' . $dtl_row2 . ':BV' . $dtl_row2);
                    $objPHPExcel->setSharedStyle($bodyStyle, 'B' . $dtl_row2 . ':BV' . $dtl_row2);
                    $objPHPExcel->mergeCells('B' . $dtl_row2 . ':F' . $dtl_row2)->setCellValue('B' . $dtl_row2, $dt_time_check_shift_1[$b]);
                    $objPHPExcel->mergeCells('G' . $dtl_row2 . ':I' . $dtl_row2)->setCellValue('G' . $dtl_row2, $dt_ph_air_shift_1[$b]);
                    $objPHPExcel->mergeCells('J' . $dtl_row2 . ':M' . $dtl_row2)->setCellValue('J' . $dtl_row2, $dt_ph_caustic_shift_1[$b]);
                    $objPHPExcel->mergeCells('N' . $dtl_row2 . ':P' . $dtl_row2)->setCellValue('N' . $dtl_row2, $dt_temp_caustic_shift_1[$b]);
                    $objPHPExcel->mergeCells('Q' . $dtl_row2 . ':T' . $dtl_row2)->setCellValue('Q' . $dtl_row2, $dt_ph_acid_shift_1[$b]);
                    $objPHPExcel->mergeCells('U' . $dtl_row2 . ':Z' . $dtl_row2)->setCellValue('U' . $dtl_row2, $dt_temp_acid_shift_1[$b]);

                    $objPHPExcel->mergeCells('AA' . $dtl_row2 . ':AC' . $dtl_row2)->setCellValue('AA' . $dtl_row2, $dt_time_check_shift_2[$b]);
                    $objPHPExcel->mergeCells('AD' . $dtl_row2 . ':AF' . $dtl_row2)->setCellValue('AD' . $dtl_row2, $dt_ph_air_shift_2[$b]);
                    $objPHPExcel->mergeCells('AG' . $dtl_row2 . ':AJ' . $dtl_row2)->setCellValue('AG' . $dtl_row2, $dt_ph_caustic_shift_2[$b]);
                    $objPHPExcel->mergeCells('AK' . $dtl_row2 . ':AM' . $dtl_row2)->setCellValue('AK' . $dtl_row2, $dt_temp_caustic_shift_2[$b]);
                    $objPHPExcel->mergeCells('AN' . $dtl_row2 . ':AQ' . $dtl_row2)->setCellValue('AN' . $dtl_row2, $dt_ph_acid_shift_2[$b]);
                    $objPHPExcel->mergeCells('AR' . $dtl_row2 . ':AW' . $dtl_row2)->setCellValue('AR' . $dtl_row2, $dt_temp_acid_shift_2[$b]);

                    $objPHPExcel->mergeCells('AX' . $dtl_row2 . ':AZ' . $dtl_row2)->setCellValue('AX' . $dtl_row2, $dt_time_check_shift_3[$b]);
                    $objPHPExcel->mergeCells('BA' . $dtl_row2 . ':BC' . $dtl_row2)->setCellValue('BA' . $dtl_row2, $dt_ph_air_shift_3[$b]);
                    $objPHPExcel->mergeCells('BD' . $dtl_row2 . ':BG' . $dtl_row2)->setCellValue('BD' . $dtl_row2, $dt_ph_caustic_shift_3[$b]);
                    $objPHPExcel->mergeCells('BH' . $dtl_row2 . ':BJ' . $dtl_row2)->setCellValue('BH' . $dtl_row2, $dt_temp_caustic_shift_3[$b]);
                    $objPHPExcel->mergeCells('BK' . $dtl_row2 . ':BN' . $dtl_row2)->setCellValue('BK' . $dtl_row2, $dt_ph_acid_shift_3[$b]);
                    $objPHPExcel->mergeCells('BO' . $dtl_row2 . ':BV' . $dtl_row2)->setCellValue('BO' . $dtl_row2, $dt_temp_acid_shift_3[$b]);
                }
                // end detail b
                ...
                $objPHPExcel->mergeCells('B'  . ($dtl_row3 + 1) . ':BV' . ($dtl_row3 + 1))->setCellValue('B' .  ($dtl_row3 + 1), "Hasil Analisa Setelah CIP");
                $objPHPExcel->mergeCells('B'  . ($dtl_row3 + 2) . ':Z' . ($dtl_row3 + 2))->setCellValue('B' .  ($dtl_row3 + 2), "Shift I");
                $objPHPExcel->mergeCells('B'  . ($dtl_row3 + 3) . ':F' . ($dtl_row3 + 3))->setCellValue('B' .  ($dtl_row3 + 3), "Time Check ");
                $objPHPExcel->mergeCells('G'  . ($dtl_row3 + 3) . ':I' . ($dtl_row3 + 3))->setCellValue('G' .  ($dtl_row3 + 3), "pH air ");
                $objPHPExcel->mergeCells('J'  . ($dtl_row3 + 3) . ':T' . ($dtl_row3 + 3))->setCellValue('J' .  ($dtl_row3 + 3), "Residu Caustic");
                $objPHPExcel->mergeCells('U'  . ($dtl_row3 + 3) . ':Z' . ($dtl_row3 + 3))->setCellValue('U' .  ($dtl_row3 + 3), "Residu Acid");

                $objPHPExcel->mergeCells('B'  . ($dtl_row3 + 4) . ':F' . ($dtl_row3 + 4))->setCellValue('B' .  ($dtl_row3 + 4), "CIP ");
                $objPHPExcel->mergeCells('G'  . ($dtl_row3 + 4) . ':I' . ($dtl_row3 + 4))->setCellValue('G' .  ($dtl_row3 + 4), "(6,5-8,5)");
                $objPHPExcel->mergeCells('J'  . ($dtl_row3 + 4) . ':T' . ($dtl_row3 + 4))->setCellValue('J' .  ($dtl_row3 + 4), "% Konsentrasi)");
                $objPHPExcel->mergeCells('U'  . ($dtl_row3 + 4) . ':Z' . ($dtl_row3 + 4))->setCellValue('U' .  ($dtl_row3 + 4), "% Konsentrasi");

                $objPHPExcel->mergeCells('AA'  . ($dtl_row3 + 2) . ':AW' . ($dtl_row3 + 2))->setCellValue('AA' .  ($dtl_row3 + 2), "Shift 2");
                $objPHPExcel->mergeCells('AA'  . ($dtl_row3 + 3) . ':AC' . ($dtl_row3 + 3))->setCellValue('AA' .  ($dtl_row3 + 3), "Time Check ");
                $objPHPExcel->mergeCells('AD'  . ($dtl_row3 + 3) . ':AF' . ($dtl_row3 + 3))->setCellValue('AD' .  ($dtl_row3 + 3), "pH air ");
                $objPHPExcel->mergeCells('AG'  . ($dtl_row3 + 3) . ':AQ' . ($dtl_row3 + 3))->setCellValue('AG' .  ($dtl_row3 + 3), "Residu Caustic");
                $objPHPExcel->mergeCells('AR'  . ($dtl_row3 + 3) . ':AW' . ($dtl_row3 + 3))->setCellValue('AR' .  ($dtl_row3 + 3), "Residu Acid");

                $objPHPExcel->mergeCells('AA'  . ($dtl_row3 + 4) . ':AC' . ($dtl_row3 + 4))->setCellValue('AA' .  ($dtl_row3 + 4), "CIP ");
                $objPHPExcel->mergeCells('AD'  . ($dtl_row3 + 4) . ':AF' . ($dtl_row3 + 4))->setCellValue('AD' .  ($dtl_row3 + 4), "(6,5-8,5)");
                $objPHPExcel->mergeCells('AG'  . ($dtl_row3 + 4) . ':AQ' . ($dtl_row3 + 4))->setCellValue('AG' .  ($dtl_row3 + 4), "% Konsentrasi)");
                $objPHPExcel->mergeCells('AR'  . ($dtl_row3 + 4) . ':AW' . ($dtl_row3 + 4))->setCellValue('AR' .  ($dtl_row3 + 4), "% Konsentrasi");

                $objPHPExcel->mergeCells('AX'  . ($dtl_row3 + 2) . ':BV' . ($dtl_row3 + 2))->setCellValue('AX' .  ($dtl_row3 + 2), "Shift 3");
                $objPHPExcel->mergeCells('AX'  . ($dtl_row3 + 3) . ':AZ' . ($dtl_row3 + 3))->setCellValue('AX' .  ($dtl_row3 + 3), "Time Check ");
                $objPHPExcel->mergeCells('BA'  . ($dtl_row3 + 3) . ':BC' . ($dtl_row3 + 3))->setCellValue('BA' .  ($dtl_row3 + 3), "pH air ");
                $objPHPExcel->mergeCells('BD'  . ($dtl_row3 + 3) . ':BN' . ($dtl_row3 + 3))->setCellValue('BD' .  ($dtl_row3 + 3), "Residu Caustic");
                $objPHPExcel->mergeCells('BO'  . ($dtl_row3 + 3) . ':BV' . ($dtl_row3 + 3))->setCellValue('BO' .  ($dtl_row3 + 3), "Residu Acid");

                $objPHPExcel->mergeCells('AX'  . ($dtl_row3 + 4) . ':AZ' . ($dtl_row3 + 4))->setCellValue('AX' .  ($dtl_row3 + 4), "CIP ");
                $objPHPExcel->mergeCells('BA'  . ($dtl_row3 + 4) . ':BC' . ($dtl_row3 + 4))->setCellValue('BA' .  ($dtl_row3 + 4), "(6,5-8,5)");
                $objPHPExcel->mergeCells('BD'  . ($dtl_row3 + 4) . ':BN' . ($dtl_row3 + 4))->setCellValue('BD' .  ($dtl_row3 + 4), "% Konsentrasi)");
                $objPHPExcel->mergeCells('BO'  . ($dtl_row3 + 4) . ':BV' . ($dtl_row3 + 4))->setCellValue('BO' .  ($dtl_row3 + 4), "% Konsentrasi");

                $objPHPExcel->setSharedStyle($DetailheaderStyle, 'B' . ($dtl_row3 + 1) . ':BV' . ($dtl_row3 + 4));
                $objPHPExcel->getStyle('B' . ($dtl_row3 + 1) . ':BV' . ($dtl_row3 + 2))->getFont()->setBold(true);
                $objPHPExcel->setSharedStyle($bodyStyleRight, 'BW' . ($dtl_row3) . ':BW' . ($dtl_row3 + 9));
                $objPHPExcel->setSharedStyle($bodyStyleLeft, 'A' . ($dtl_row3) . ':A' . ($dtl_row3 + 9));

                $dtl_row4 = $dtl_row3 + 4;

                for ($c = 0; $c <= 0; $c++) {
                    $dtl_row4++;
                    $objPHPExcel->getRowDimension($dtl_row4)->setRowHeight(25);
                    $dt_time_check2_shift_1     [$c] = $time_check2_shift_1     [$c] ?? "";
                    $dt_ph_shift_1              [$c] = $ph_shift_1              [$c] ?? "";
                    $dt_residu_caustic_shift_1  [$c] = $residu_caustic_shift_1  [$c] ?? "";
                    $dt_residu_acid_shift_1     [$c] = $residu_acid_shift_1     [$c] ?? "";
                

                    $dt_time_check2_shift_2     [$c] = $time_check2_shift_2     [$c] ?? "";
                    $dt_ph_shift_2              [$c] = $ph_shift_2              [$c] ?? "";
                    $dt_residu_caustic_shift_2  [$c] = $residu_caustic_shift_2  [$c] ?? "";
                    $dt_residu_acid_shift_2     [$c] = $residu_acid_shift_2     [$c] ?? "";
                

                    $dt_time_check2_shift_3     [$c] = $time_check2_shift_3     [$c] ?? "";
                    $dt_ph_shift_3              [$c] = $ph_shift_3              [$c] ?? "";
                    $dt_residu_caustic_shift_3  [$c] = $residu_caustic_shift_3  [$c] ?? "";
                    $dt_residu_acid_shift_3     [$c] = $residu_acid_shift_3     [$c] ?? "";
                

                    $objPHPExcel->setSharedStyle($bodyStyle, 'B' . $dtl_row4 . ':BV' . $dtl_row4);

                    $objPHPExcel->mergeCells('B' . $dtl_row4  . ':F' . $dtl_row4)->setCellValue('B' . $dtl_row4, $dt_time_check2_shift_1[$c]);
                    $objPHPExcel->mergeCells('G' . $dtl_row4  . ':I' . $dtl_row4)->setCellValue('G' . $dtl_row4, $dt_ph_shift_1[$c]);
                    $objPHPExcel->mergeCells('J' . $dtl_row4  . ':T' . $dtl_row4)->setCellValue('J' . $dtl_row4, $dt_residu_caustic_shift_1[$c]);
                    $objPHPExcel->mergeCells('U' . $dtl_row4  . ':Z' . $dtl_row4)->setCellValue('U' . $dtl_row4, $dt_residu_acid_shift_1[$c]);

                    $objPHPExcel->mergeCells('AA' . $dtl_row4  . ':AC' . $dtl_row4)->setCellValue('AA' . $dtl_row4, $dt_time_check2_shift_2[$c]);
                    $objPHPExcel->mergeCells('AD' . $dtl_row4  . ':AF' . $dtl_row4)->setCellValue('AD' . $dtl_row4, $dt_ph_shift_2[$c]);
                    $objPHPExcel->mergeCells('AG' . $dtl_row4  . ':AQ' . $dtl_row4)->setCellValue('AG' . $dtl_row4, $dt_residu_caustic_shift_2[$c]);
                    $objPHPExcel->mergeCells('AR' . $dtl_row4  . ':AW' . $dtl_row4)->setCellValue('AR' . $dtl_row4, $dt_residu_acid_shift_2[$c]);

                    $objPHPExcel->mergeCells('AX' . $dtl_row4  . ':AZ' . $dtl_row4)->setCellValue('AX' . $dtl_row4, $dt_time_check2_shift_3[$c]);
                    $objPHPExcel->mergeCells('BA' . $dtl_row4  . ':BC' . $dtl_row4)->setCellValue('BA' . $dtl_row4, $dt_ph_shift_3[$c]);
                    $objPHPExcel->mergeCells('BD' . $dtl_row4  . ':BN' . $dtl_row4)->setCellValue('BD' . $dtl_row4, $dt_residu_caustic_shift_3[$c]);
                    $objPHPExcel->mergeCells('BO' . $dtl_row4  . ':BV' . $dtl_row4)->setCellValue('BO' . $dtl_row4, $dt_residu_acid_shift_3[$c]);
                }
                ...
                $table4 = $dtl_row4;
                $objPHPExcel->mergeCells('B'  . ($table4 + 1) . ':BV' . ($table4 + 1))->setCellValue('B' .  ($table4 + 1), "CATATAN KETIDAKSESUAIAN");
                $objPHPExcel->mergeCells('B'  . ($table4 + 2) . ':H' . ($table4 + 2))->setCellValue('B' .  ($table4 + 2), "Jam");
                $objPHPExcel->mergeCells('I'  . ($table4 + 2) . ':W' . ($table4 + 2))->setCellValue('I' .  ($table4 + 2), "Uraian ketidaksesuaian");
                $objPHPExcel->mergeCells('X'  . ($table4 + 2) . ':AQ' . ($table4 + 2))->setCellValue('X' .  ($table4 + 2), "Tindakan Perbaikan");
                $objPHPExcel->mergeCells('AR'  . ($table4 + 2) . ':AZ' . ($table4 + 2))->setCellValue('AR' .  ($table4 + 2), "Nama");
                $objPHPExcel->mergeCells('BA'  . ($table4 + 2) . ':BH' . ($table4 + 2))->setCellValue('BA' .  ($table4 + 2), "Paraf");
                $objPHPExcel->mergeCells('BI'  . ($table4 + 2) . ':BV' . ($table4 + 2))->setCellValue('BI' .  ($table4 + 2), "Ket");

                $objPHPExcel->setSharedStyle($DetailheaderStyle, 'B' . ($table4 + 1) . ':BV' . ($table4 + 2));
                $objPHPExcel->getStyle('B' . ($table4 + 1) . ':BV' . ($table4 + 2))->getFont()->setBold(true);
                $objPHPExcel->setSharedStyle($noborderStyle, 'B' . ($table4 + 3) . ':BV' . ($table4 + 3));
                $objPHPExcel->setSharedStyle($bodyStyleRight, 'BW' . ($table4 + 3) . ':BW' . ($table4 + 6));
                $objPHPExcel->setSharedStyle($bodyStyleLeft, 'A' . ($table4 + 3) . ':A' . ($table4 + 6));

                $keterangan1 = $table4 + 2;
                for ($d = 0; $d <= 3; $d++) {
                    $keterangan1++;
                    $objPHPExcel->getRowDimension($keterangan1)->setRowHeight(25);
                    $dt_jam               [$d]  = $dtl_d_jam[$d]                ?? "";
                    $dt_uraian            [$d]  = $dtl_d_uraian[$d]             ?? "";
                    $dt_tindakan          [$d]  = $dtl_d_tindakan[$d]           ?? "";
                    $dt_pj_nama           [$d]  = $dtl_d_pj_nama[$d]            ?? "";
                    $dt_pj_personalstatus [$d]  = $dtl_d_pj_personalstatus[$d]  ?? "";
                    $dt_pj_personalid     [$d]  = $dtl_d_pj_personalid[$d]      ?? "";
                    $dt_paraf             [$d]  = $dtl_d_paraf[$d]              ?? "";
                    $dt_keterangan        [$d]  = $dtl_d_keterangan[$d]         ?? "";
                    ...
                    $objPHPExcel->mergeCells('X' .  $keterangan1 . ':AQ' . $keterangan1)->setCellValue('X' .  $keterangan1, $dt_tindakan[$d]);
                    ...
                    $objPHPExcel->mergeCells('BI' . $keterangan1 . ':BV' . $keterangan1)->setCellValue('BI' . $keterangan1, $dt_keterangan[$d]);
            ...
            $objPHPExcel->getStyle('R' . ($start_row3 + 1) . ':BW' . ($start_row3 + 1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            $objPHPExcel->mergeCells('R' . ($start_row3 + 1) . ':BW' . ($start_row3 + 1))->setCellValue('R' . ($start_row3 + 1), $this->frmnm . '-' . $this->frmver);
            $objPHPExcel->setSharedStyle($footerStyleLeftbottom, 'A' . ($start_row3 + 1) . ':Q' . ($start_row3 + 1));
            $objPHPExcel->setSharedStyle($footerStyleRightbottom, 'R' . ($start_row3 + 1) . ':BW' . ($start_row3 + 1));
